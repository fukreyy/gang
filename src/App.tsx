...
// Global helper: convert Cloudinary HLS URL → MP4 fallback
const toMp4Url = (url: string): string => {
  if (!url) return '';
  if (url.includes("cloudinary.com") && url.includes("/video/upload/")) {
    return url
      .replace(/\/sp_hd\//g, "/")
      .replace(/\.m3u8(\?.*)?$/i, ".mp4$1")
      .replace(/\.(webm|mkv|avi|mov)(\?.*)?$/i, ".mp4$2");
  }
  return url;
};
...
const ANIME_PROVIDERS = [
  ...
  },
  {
    name: "Filmu",
    getUrl: (anilistId: number, ep: number, _malId?: number | null) =>
      `https://embed.filmu.in/anime/${anilistId}/1/${ep}`,
  },
];
    ...
    function App() {
      ...
      useEffect(() => {
        ...
        // Block _blank links and ad redirects
        ...
        const blockLocationChange = (url: string | URL | null | undefined) => {
          if (url && !String(url).includes(location.hostname)) {
  ...
  setPushLoading(false);
      ...
      useEffect(() => {
        ...
        // Reset video element fully (fixes re-play issue)
        try { video.pause(); } catch {}
        video.removeAttribute('src');
        try { video.load(); } catch {}

        let hls: Hls | null = null;
        const mp4Fallback = toMp4Url(currentVideoSrc);

        const fallbackToMp4 = () => {
          if (!mp4Fallback || video.src === mp4Fallback) return;
          console.log("🎬 HLS failed, falling back to MP4:", mp4Fallback);
          try { hls?.destroy(); } catch {}
          video.src = mp4Fallback;
          video.play().catch(() => {});
        };

        if (Hls.isSupported() && currentVideoSrc.includes(".m3u8")) {
          hls = new Hls();
          hls.loadSource(currentVideoSrc);
          hls.attachMedia(video);
          hls.on(Hls.Events.MANIFEST_PARSED, () => video.play().catch(() => {}));
          hls.on(Hls.Events.ERROR, (_e, data) => {
            if (data.fatal) { console.warn("HLS fatal:", data.type, data.details); fallbackToMp4(); }
          });
          ...
          video.src = mp4Fallback || currentVideoSrc;
          video.play().catch(() => {});
        }
      ...
      useEffect(() => {
        const handlePopState = (e: PopStateEvent) => {
          ...
          if (showVideoPlayer) { setShowVideoPlayer(false); setCurrentVideoSrc('');  setIsPlaying(false); setCurrentEpisode(null); window.history.pushState({ nav }, '', '#' + encodeURIComponent(nav)); return; }
...
function OnlineSection({ onGoHome }: { onGoHome: () => void }) {
      ...
      const [comingFilter, setComingFilter] = useState<'today' | 'week' | 'month'>('week');
      const searchTimeoutRef = useRef<ReturnType<typeof setTimeout>>();
      ...
      const fetchComingSoon = async () => {
        ...
        try {
          ...
          const [movRes, tvRes] = await Promise.all([
            tmdbFetch(`/discover/movie?language=en-US&sort_by=release_date.asc&release_date.gte=${today}&release_date.lte=${in30}&region=IN`),
            tmdbFetch(`/discover/tv?language=en-US&sort_by=first_air_date.asc&first_air_date.gte=${today}&first_air_date.lte=${in30}`),
          ]);
          const movies = (movRes.results || []).map((m: TMDBMovie) => ({ ...m, media_type: 'movie' as const }));
          const tv = (tvRes.results || []).map((t: TMDBMovie) => ({ ...t, media_type: 'tv' as const }));
      ...
      const fetchTVDetails = async (id: number) => {
        try {
          ...
          const s1data = await tmdbFetch(`/tv/${id}/season/1?language=en-US`);
          const airedS1 = (s1data.episodes || []).filter((ep: any) => ep.air_date && new Date(ep.air_date) <= today);
          setEpisodeCount(airedS1.length || 1);
        } catch (e) { console.error(e); }
      };
      ...
      const fetchEpisodeCount = async (id: number, season: number) => {
        try {
          ...
          const today = new Date();
          const aired = (data.episodes || []).filter((ep: any) => ep.air_date && new Date(ep.air_date) <= today);
          setEpisodeCount(aired.length || 1);
        } catch (e) { console.error(e); }
      };
      ...
      const openItem = async (item: TMDBMovie) => {
        ...
        if (isTV(item)) {
          ...
          await fetchEpisodeCount(item.id, s);
...
const startStream = (item: TMDBMovie, season = selectedSeason, episode = selectedEpisode, providerIndex = selectedProvider) => {
  if (!isReleased(item)) { alert(`🗓️ "${getTitle(item)}" is not released yet.`); return; }
  ...
  const url = buildStreamUrl(item, season, episode, providerIndex);
    ...
    function TMDBCard({ item, isTV, getTitle, getYear, getGenres, onClick }: {
      ...
      const releaseLabel = !released ? new Date(item.release_date || item.first_air_date || '').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : null;
      return (
        <button onClick={released ? onClick : undefined} className={`group relative flex-shrink-0 text-left transition-all duration-300 ${released ? 'hover:scale-105' : 'opacity-60 cursor-default'}`}>
          <div className="relative rounded-xl overflow-hidden bg-gray-800 aspect-[2/3]">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              {released && <div className="w-14 h-14 bg-orange-500/90 rounded-full flex items-center justify-center"><PlayIcon /></div>}
            </div>
            {!released && releaseLabel && (
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-black/80 text-center">
                <p className="text-xs font-bold text-orange-400">🗓️ {releaseLabel}</p>
              </div>
            )}
  ...
  onGoHome: () => void;
      ...
      useEffect(() => {
        ...
        // MutationObserver to remove ad overlays
        const observer = new MutationObserver(() => {
          document.querySelectorAll(
            'iframe[src*="ad"], div[class*="ad-"], div[id*="ad-"], div[class*="popup"], div[class*="overlay"], [data-ad]'
...
function LiveTVSection() {
  ...
  const openChannel = (ch: { name: string; id: number }) => {
    ...
    setTimeout(async () => {
      try {
        const el = document.querySelector('.fixed.inset-0.z-\\[60\\]') as HTMLElement
...
// ─── Countdown Card ───────────────────────────────────────────────────────────
function CountdownCard({ item, getTitle, getYear, getGenres }: {
  item: TMDBMovie;
  getTitle: (i: TMDBMovie) => string;
  getYear: (i: TMDBMovie) => string;
  getGenres: (i: TMDBMovie) => string[];
}) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-800/60 hover:bg-gray-800 rounded-2xl transition-colors">
      <div className="flex-shrink-0 w-14 h-20 sm:w-16 sm:h-24 rounded-xl overflow-hidden bg-gray-700 relative">
        {item.poster_path
          ? <img src={`https://image.tmdb.org/t/p/w500${item.poster_path}`} alt={getTitle(item)} className="w-full h-full object-cover" />
          : <div className="w-full h-full flex items-center justify-center text-2xl">{isTV ? '📺' : '🎬'}</div>}
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-bold text-sm sm:text-base truncate">{getTitle(item)}</p>
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          <span className="text-xs text-gray-400">{getYear(item)}</span>
          <span className="text-xs text-gray-400">{isTV ? '📺 Series' : '🎬 Movie'}</span>
          {getGenres(item).slice(0,2).map(g => <span key={g} className="text-xs bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full">{g}</span>)}
        </div>
        <p className="text-xs text-gray-500 mt-1">
          {releaseDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}
        </p>
      </div>
      <div className={cn(
        'flex-shrink-0 px-3 py-2 rounded-xl text-center min-w-[80px] border font-mono',
        urgent
          ? 'bg-red-500/20 border-red-500/50 text-red-400 animate-pulse'
          : 'bg-orange-500/10 border-orange-500/30 text-orange-400'
      )}>
        <p className="text-[10px] text-gray-500 mb-0.5">{urgent ? '🔴 TODAY' : '⏳ IN'}</p>
